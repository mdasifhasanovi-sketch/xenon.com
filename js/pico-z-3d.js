...
(function () {
    ...
    spawn(TOTAL_PARTICLES - hookParticles, () => {
        const r = 0.5 * Math.pow(Math.random(), 1 / 3);
        const th = Math.random() * Math.PI * 2;
    ...
    // PHYSICS CONFIG (Igloo Style)
    // Fluid, Soft, Flowing
    const FLUID_SPEED = 0.5;
    const REPEL_RADIUS = 1.0;
    const REPEL_FORCE = 3.0; // Strong but soft
    const RETURN_Spring = 0.01; // Very weak return
    const DAMPING = 0.95;
    ...
    function animate() {
        ...
        for (let i = 0; i < TOTAL_PARTICLES; i++) {
            ...
            velocities[i3] += flowX * 0.005 * FLUID_SPEED;
            velocities[i3 + 1] += flowY * 0.005 * FLUID_SPEED;
            velocities[i3 + 2] += flowZ * 0.005 * FLUID_SPEED;
